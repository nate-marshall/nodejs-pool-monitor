...
mqttService.on('connect', () => {
  ...
  mqttService.subscribe(config.mqtt.topics.waterFlow, (err) => {
    if (err)
      ...
      logService.debug(
        `Subscribed to Water Flow topic: ${config.mqtt.topics.waterFlow}`
      );
  });
...
mqttService.on('message', (topic, message) => {
  ...
  try {
    ...
    if (topic === config.mqtt.topics.orp) {
      ...
      if (
        poolState.rpm > config.monitoring.pumpRpmSpeed &&
...
async function sendRemResetCommand() {
  try {
    await axios.put(
      `${config.remController.url}/config/reset`,
      {
...
async function sendFlowSwitchResetCommand() {
  try {
    await axios.put(
      `${config.remController.url}/config/gpio/pin/1/29`,
      {
...
function checkPumpAndWaterFlow(currentTime) {
  ...
  logService.debug(
    `Checking conditions: RPM=${poolState.rpm}, Water Flow=${poolState.waterFlow}, Threshold=${config.monitoring.pumpRpmSpeed}`
  );

  if (timeSinceLastAlert < throttleDuration) {
    logService.debug(
      `Reset command throttled for ${process.env.ALERT_THROTTLE_TIMER || 30} seconds. Waiting before sending again.`
    );
    return;
  }
  ...
  if (
    poolState.rpm > config.monitoring.pumpRpmSpeed &&
    poolState.waterFlow === 'off'
  ) {
    logService.warn(
      'Pump IS running but water flow is NOT detected. Resetting flow switch.'
    );
    ...
    poolState.rpm < config.monitoring.pumpRpmSpeed &&
    poolState.waterFlow === 'on'
  ) {
    logService.warn(
      'Pump IS running at low speed and water flow IS detected. Resetting flow switch.'
    );
    sendFlowSwitchResetCommand();
    lastAlertTime = currentTime;
    ...
    logService.warn(
      'Pump is NOT running and water flow is off. Stopping monitoring.'
    );
    stopMonitoring();
    ...
    logService.warn(
      'Pump and water flow are below thresholds. Stopping monitoring.'
    );
    stopMonitoring();
  } else {
...
function startMonitoring() {
  ...
  monitoringIntervalId = setInterval(async () => {
    ...
    if (failureCount >= maxFailures && timeSinceLastAlert > throttleDuration) {
      try {
        await sendRemAlertAndReset(
          'ORP and pH levels have not changed.',
          orpLevel,
          phLevel
        );
        lastAlertTime = currentTime;
    ...
    logService.debug(
      `Checking pump and water flow: RPM=${poolState.rpm}, Water Flow=${poolState.waterFlow}`
    );
  ...
  logService.info(
    'Pump running at ' +
      poolState.rpm +
      ' RPM and will start monitoring with a delay of ' +
      delayInSeconds +
      ' seconds.'
  );
}
